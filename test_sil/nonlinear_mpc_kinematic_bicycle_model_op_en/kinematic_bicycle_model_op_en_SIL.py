...
def main():
    ...
    times, px_reference_path, py_reference_path, yaw_reference = interpolate_path_csv(
        input_path="./external_libraries/MCAP_python_mpc/sample/nonlinear/support/office_area_RRT_path_data.csv",
        delta_time=delta_time,
        total_time=simulation_time
    )

    q0_reference_path = np.cos(yaw_reference * 0.5)
    q3_reference_path = np.sin(yaw_reference * 0.5)
    ...
    for i in range(round(simulation_time / delta_time)):
        ...
        for j in range(Np):
            ...
            reference[0, j] = px_reference_path[index, 0]
            reference[1, j] = py_reference_path[index, 0]
            reference[2, j] = q0_reference_path[index, 0]
            reference[3, j] = q3_reference_path[index, 0]
