...
def main():
    ...
    from test_sil.adaptive_mpc_two_wheel_vehicle_constraints_delay import AdaptiveMpcTwoWheelVehicleConstraintsDelaySIL
    AdaptiveMpcTwoWheelVehicleConstraintsDelaySIL.initialize()
    ...
    for i in range(round(simulation_time / sim_delta_time)):
        ...
        U_cpp = AdaptiveMpcTwoWheelVehicleConstraintsDelaySIL.update_manipulation(
            ref, y_measured)

        tester.expect_near(
            u_from_mpc, U_cpp, NEAR_LIMIT,
            "Adaptive MPC two wheel vehicle model constraints delay SIL, check update_manipulation.")
