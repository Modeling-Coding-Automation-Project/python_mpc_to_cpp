...
It also includes the `NonlinearMPC_OptimizationEngineDeploy` class with a static method
`generate_Nonlinear_MPC_cpp_code` that takes a `NonlinearMPC_OptimizationEngine` object
and generates the corresponding C++ code files using ALM/PM optimization engine.
"""
...
from external_libraries.python_optimization_to_cpp.optimization_utility.optimization_engine_matrix_utility_deploy import OptimizationEngine_MatrixUtilityDeploy
...
class NonlinearMPC_OptimizationEngineDeploy:
    """
    NonlinearMPC_OptimizationEngineDeploy

    This class provides static methods for deploying a
      nonlinear Model Predictive Controller (MPC) using the ALM/PM
        optimization engine by generating corresponding
          C++ code from a Python-based nonlinear MPC object.

    Methods
    -------
    generate_Nonlinear_MPC_cpp_code(nonlinear_mpc: NonlinearMPC_OptimizationEngine, file_name=None)
        Generates C++ header files for the provided nonlinear MPC object,
          including its Kalman filter, cost matrices, and all required type definitions.
        ...
        - Relies on auxiliary deployment utilities for Kalman filter
          and cost matrices code generation.
        - Uses ALM/PM optimization engine with PANOC as the inner solver,
          which only requires first-order gradient information.
    """
    ...
    def generate_Nonlinear_MPC_cpp_code(
        ...
        """
        Generates C++ header files for deploying a nonlinear Model Predictive Controller (MPC)
        using ALM/PM optimization engine with an Extended Kalman Filter (EKF)
        and associated cost matrices.
        ...
        Notes:
            ...
            - The generated C++ code assumes the existence of certain type definitions
                and namespaces (e.g., PythonNumpy, PythonControl, PythonMPC).
            - Deep copies of cost matrices are made to avoid modifying the original object.
            - The HasOutputConstraints template parameter is determined from the
                nonlinear_mpc object's _has_output_constraints attribute.
        """
        ...
        # %% generate cost matrices code
        cost_matrices = copy.deepcopy(nonlinear_mpc.cost_matrices)

        cost_matrices_code_names = OptimizationEngine_MatrixUtilityDeploy.generate_cpp_code(
            cost_matrices=cost_matrices,
            file_name=caller_file_name_no_extension,
        )
        deployed_file_names.extend(cost_matrices_code_names)
        ...
        has_output_constraints = nonlinear_mpc._has_output_constraints
        has_output_constraints_str = "true" if has_output_constraints else "false"

        lbfgs_memory = 5  # PANOC_LBFGS_MEMORY_DEFAULT
        ...
        code_text += f"constexpr bool HAS_OUTPUT_CONSTRAINTS = {has_output_constraints_str};\n\n"

        code_text += f"constexpr std::size_t LBFGS_MEMORY = {lbfgs_memory};\n\n"
        ...
        code_text += f"using type = NonlinearMPC_OptimizationEngine_Type<\n" + \
            "    EKF_Type, Cost_Matrices_Type, HAS_OUTPUT_CONSTRAINTS, LBFGS_MEMORY>;\n\n"
        ...
        code_text += f"    auto nonlinear_mpc = make_NonlinearMPC_OptimizationEngine<\n" + \
            f"        EKF_Type, Cost_Matrices_Type, HAS_OUTPUT_CONSTRAINTS, LBFGS_MEMORY>(\n" + \
            f"        kalman_filter, cost_matrices, delta_time, X_initial);\n\n"
