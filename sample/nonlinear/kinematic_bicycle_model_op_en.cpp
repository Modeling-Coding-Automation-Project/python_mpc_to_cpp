/**
 * @file kinematic_bicycle_model_op_en.cpp
 *
 * @brief Example of Nonlinear MPC for a kinematic bicycle model with nonlinear
 * dynamics using the PANOC/ALM optimization engine.
 *
...
// You must run "kinematic_bicycle_model_op_en.py" to use
// "kinematic_bicycle_model_op_en_nonlinear_mpc.hpp"
#include "kinematic_bicycle_model_op_en_nonlinear_mpc.hpp"
...
int main(void) {
  ...
  constexpr std::size_t STATE_SIZE =
      kinematic_bicycle_model_op_en_nonlinear_mpc::STATE_SIZE;
  constexpr std::size_t INPUT_SIZE =
      kinematic_bicycle_model_op_en_nonlinear_mpc::INPUT_SIZE;
  constexpr std::size_t OUTPUT_SIZE =
      kinematic_bicycle_model_op_en_nonlinear_mpc::OUTPUT_SIZE;
  ...
  kinematic_bicycle_model_op_en_nonlinear_mpc::ReferenceTrajectory_Type
      reference_trajectory;

  // You must run "kinematic_bicycle_model_op_en.py" to generate
  // "reference_path.csv".
  ...
  std::string csv_path = source_dir + "/cpp_run_data_op_en.csv";
  ...
  if (!ofs) {
    ...
    ofs << "px,py,yaw,v,delta,outer_iteration,inner_iteration\n";
  ...
  for (std::size_t step = 0; step < MAX_STEP; ++step) {
    /* system response */
    X = kinematic_bicycle_model_op_en_nonlinear_mpc_ekf_state_function::
    ...
    Y = kinematic_bicycle_model_op_en_nonlinear_mpc_ekf_measurement_function::
    ...
    std::cout << "outer_iteration: " << outer_solver_iteration << ", ";
    std::cout << "inner_iteration: " << inner_solver_iteration << ", ";
    ...
    if (ofs) {
      ofs << Y(0, 0) << ',' << Y(1, 0) << ',' << yaw << ',' << U(0, 0) << ','
          << U(1, 0) << ',' << outer_solver_iteration << ','
          << inner_solver_iteration << '\n';
    }
  }
