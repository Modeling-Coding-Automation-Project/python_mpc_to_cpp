        ...
        if os.path.exists(cpp_csv_path):
            ...
            if cpp_data is not None:
                ...
                print(
                    ...
                    f" (cols={px_cpp.shape[0] if px_cpp is not None else 0})")
...
def main():
    ...
    if cpp_run_data_exists:
        plotter.assign("outer_solver_iteration_cpp", row=0, column=0,
                       position=(2, 1), x_sequence=times,
                       label="outer_solver_iteration_cpp")
        plotter.assign("inner_solver_iteration_cpp", row=0, column=0,
                       position=(2, 1), x_sequence=times,
