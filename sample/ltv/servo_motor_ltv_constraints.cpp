/**
 * @file servo_motor_ltv_constraints.cpp
...
/* CAUTION */
// You need to run "servo_motor_ltv_constraints.py" before running this code.
...
int main(void) {
  ...
  /* Define parameters */
  servo_motor_ltv_constraints_parameters::Parameter plant_parameters;
  ...
  for (std::size_t sim_step = 0; sim_step < 800; ++sim_step) {
    if (!parameter_changed && sim_step >= PARAMETER_CHANGE_STEP) {
      ...
      servo_motor_ltv_constraints_mpc_state_space_updater::
