...
template <typename T>
void check_Nonlinear_MPC_Optimization_Engine(void) {
    using namespace PythonNumpy;
    using namespace PythonControl;
    using namespace PythonMPC;
    ...
    /* 定義 */
    constexpr std::size_t NP = kinematic_bicycle_model_open_cost_matrices::NP;

    constexpr std::size_t INPUT_SIZE = kinematic_bicycle_model_open_cost_matrices::INPUT_SIZE;
    constexpr std::size_t STATE_SIZE = kinematic_bicycle_model_open_cost_matrices::STATE_SIZE;
    constexpr std::size_t OUTPUT_SIZE = kinematic_bicycle_model_open_cost_matrices::OUTPUT_SIZE;
    ...
    using Cost_Matrices_Type = kinematic_bicycle_model_open_cost_matrices::type<T>;

    auto kalman_filter = kinematic_bicycle_model_nmpc_ekf::make<T>();

    auto cost_matrices = kinematic_bicycle_model_open_cost_matrices::make<T>();

    using Nonlinear_MPC_Type = NonlinearMPC_OptimizationEngine_Type<EKF_Type, Cost_Matrices_Type>;

    NonlinearMPC_OptimizationEngine_Type<EKF_Type, Cost_Matrices_Type> nonlinear_mpc
        = make_NonlinearMPC_OptimizationEngine(kalman_filter, cost_matrices, delta_time, X_initial);
    ...
    tester.expect_near(nonlinear_mpc.U_horizon.matrix.data,
        nonlinear_mpc_copy.U_horizon.matrix.data, NEAR_LIMIT_STRICT,
        "check Nonlinear MPC OptEng, copy, U_horizon(0, 0).");
...
int main(void) {
    ...
    check_Nonlinear_MPC_Optimization_Engine<double>();
